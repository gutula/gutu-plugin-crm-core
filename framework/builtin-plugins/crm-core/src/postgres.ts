export type CrmCoreSqlOptions = {
  schemaName?: string;
  dropSchema?: boolean;
};

export function buildCrmCoreMigrationSql(options: CrmCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "crm_core", "schemaName");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getCrmCoreLookupIndexName()} ON ${schemaName}.primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getCrmCoreStatusIndexName()} ON ${schemaName}.exception_records (tenant_id, status, severity);`
  ];
}

export function buildCrmCoreRollbackSql(options: CrmCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "crm_core", "schemaName");
  const dropSchema = options.dropSchema ?? schemaName !== "crm_core";
  return [
...
export function getCrmCoreLookupIndexName(): string {
  return "crm_core_primary_lookup_idx";
}

export function getCrmCoreStatusIndexName(): string {
  return "crm_core_exception_status_idx";
}
