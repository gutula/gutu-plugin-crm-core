export type CrmCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildCrmCoreSqliteMigrationSql(options: CrmCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "crm_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getCrmCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getCrmCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildCrmCoreSqliteRollbackSql(options: CrmCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "crm_core_");
  return [
...
export function getCrmCoreSqliteLookupIndexName(tablePrefix = "crm_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getCrmCoreSqliteStatusIndexName(tablePrefix = "crm_core_"): string {
