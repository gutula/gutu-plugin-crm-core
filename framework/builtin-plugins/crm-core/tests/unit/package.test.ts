...
import { businessFlowDefinitions, captureCrmLead, advanceOpportunity, prepareSalesHandoff, placeRecordOnHold, releaseRecordHold, amendRecord, reverseRecord } from "../../src/flows/catalog";
...
describe("plugin manifest", () => {
  it("keeps a stable package id and business contract surface", () => {
    expect(manifest.id).toBe("crm-core");
    expect(manifest.kind).toBe("plugin");
    expect(manifest.publicCommands).toContain("crm.leads.capture");
    expect(manifest.publicEvents).toContain("crm.lead-captured.v1");
  });
});
...
describe("domain catalog", () => {
  it("keeps ERPNext parity references and operational surfaces visible", () => {
    ...
    expect(typeof captureCrmLead).toBe("function");
    expect(typeof advanceOpportunity).toBe("function");
    expect(typeof prepareSalesHandoff).toBe("function");
